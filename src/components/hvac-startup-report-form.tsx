...
interface HvacStartupReportFormProps {
    report: HvacStartupReport;
...
export function HvacStartupReportForm({ report, workOrder, technicians, onFormSaved, onCancel }: HvacStartupReportFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        toast({ title: 'HVAC Start-Up Report Updated' });
        onFormSaved();
        ...
        <CardContent className="p-6 sm:p-8">
          ...
          <main className="space-y-4">
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-b pb-4">
                <div className="space-y-1">
                    <label className="text-sm font-medium">Work Order</label>
                     <Input readOnly disabled value={workOrder ? `${workOrder.id} - ${workOrder.jobName}` : (report.workOrderId || 'N/A')} />
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                    ...
                    <Input readOnly disabled value={workOrder?.workSite?.name || formState.site || ''} />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 pt-4">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 pt-4">
                <GroupedLabeledInput label="Condenser: Suction Pressure" values={['suctionPressure_S1', 'suctionPressure_S2', 'suctionPressure_S3', 'suctionPressure_S4'].map(k => formState[k as keyof HvacStartupReport] as string || '')} onChange={(i, v) => handleGroupedInputChange('suctionPressure', i, v)} labels={['S-1', 'S-2', 'S-3', 'S-4']} />
                <GroupedLabeledInput label="Condenser: Discharge Pressure" values={['dischargePressure_S1', 'dischargePressure_S2', 'dischargePressure_S3', 'dischargePressure_S4'].map(k => formState[k as keyof HvacStartupReport] as string || '')} onChange={(i, v) => handleGroupedInputChange('dischargePressure', i, v)} labels={['S-1', 'S-2', 'S-3', 'S-4']} />
                <GroupedLabeledInput label="Condenser: Crankcase Heater AMP" values={['crankcaseHeaterAmp_S1', 'crankcaseHeaterAmp_S2', 'crankcaseHeaterAmp_S3', 'crankcaseHeaterAmp_S4'].map(k => formState[k as keyof HvacStartupReport] as string || '')} onChange={(i, v) => handleGroupedInputChange('crankcaseHeaterAmp', i, v)} labels={['S-1', 'S-2', 'S-3', 'S-4']} />
                <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 pt-4">
                <div>
                 ...
                 <div>
                    ...
                    <div className="p-4 border border-t-0 space-y-2">
                        <GroupedLabeledInput label="RAT | SAT Temp °F" values={['gasHeating_RAT_SAT_S1', 'gasHeating_RAT_SAT_S2', 'gasHeating_RAT_SAT_S3', 'gasHeating_RAT_SAT_S4'].map(k => formState[k as keyof HvacStartupReport] as string || '')} onChange={(i, v) => handleGroupedInputChange('gasHeating_RAT_SAT', i, v)} labels={['S-1', 'S-2', 'S-3', 'S-4']} />
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 pt-4">
                <div>
                    ...
                    <div className="p-4 border border-t-0 space-y-2">
                        <GroupedLabeledInput label="RAT | SAT Temp °F" values={['electricHeating_RAT_SAT_S1', 'electricHeating_RAT_SAT_S2', 'electricHeating_RAT_SAT_S3', 'electricHeating_RAT_SAT_S4'].map(k => formState[k as keyof HvacStartupReport] as string || '')} onChange={(i, v) => handleGroupedInputChange('electricHeating_RAT_SAT', i, v)} labels={['S-1', 'S-2', 'S-3', 'S-4']} />
                    </div>
                </div>
                <div>
