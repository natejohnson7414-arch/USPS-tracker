...
export function TrainingRecordForm({ record, onFormSaved, onCancel }: TrainingRecordFormProps) {
  ...
  useEffect(() => {
    if (record) {
      ...
      setBasUserName(record.basUserName || '');
      setBasPassword(record.basPassword || '');
      setDate(record.date ? new Date(record.date) : undefined);
      setTrainerSignatureUrl(record.trainerSignatureUrl || null);
      setAttendees(record.attendees.length > 0 ? record.attendees : [{ id: `attendee-${Date.now()}`, name: '' }]);
